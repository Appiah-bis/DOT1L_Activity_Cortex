...
Remove Mki67 to be sure cells clustering is not affected by cell cycle phase
```{r removeercc}
#sce_hvg.seurat <- sce_hvg.seurat[grep("^(Mki)",row.names(sce_hvg.seurat),invert=TRUE),]
```
...
Cluster number 2 seems to be missing under EPZ condition, but we don´t know whether the difference is due to the plates the samples were processed on or whether it´s a biological effect.
...
```{r}
plots <- VlnPlot(sce_hvg.seurat, features = c("Hopx","Mki67"), split.by = "condition", group.by = "seurat_clusters", 
