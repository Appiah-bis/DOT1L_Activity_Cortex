...
#data <- readRDS("AMI_ALL2.RData")

sc2$condition <- "NA"
sc2$condition[which(colnames(sc2) %in% EUE14DMS)] <- "DMS24"
sc2$condition[which(colnames(sc2) %in% EUE14EPZ)] <- "EPZ24"
#sce$condition[which(colnames(sce) %in% NA)] <- "OUT"
#sce$condition[epz] <- "EPZ"
...
```{r}
plots <- VlnPlot(sce_hvg.seurat, features = c("Sox2","Tubb3"), split.by = "condition", group.by = "seurat_clusters", 
...
#plot_tree(umap, clus, l1, threeD = TRUE)
