...
#sce <- as.matrix(readRDS("Bismark.RData"))
sce_hvg_1 <- readRDS("sce_hvg.RDS")

exprMat <- counts(sce_hvg_1)
cellInfo <- colData(sce_hvg_1)
#cellInfo <- data.frame(seuratCluster=Idents())

exprMat <- counts(sce_hvg)
cellInfo <- colData(sce_hvg)
#cellInfo <- data.frame(seuratCluster=Idents())



#cellInfo <- get_cellAnnotation(exprMat)
#close_loom(loom)
...
# Show TF expression:
par(mfrow=c(2,3))
AUCell::AUCell_plotTSNE(tSNE_scenic$Y, exprMat_log, aucell_regulonAUC[onlyNonDuplicatedExtended(rownames(aucell_regulonAUC))[c("Sox4", "Sox9")],], plots="Expression")
```
...
cellInfo2 <- data.frame(seuratCluster=Idents(sce_hvg.seurat))
